...
st.set_page_config(page_title="Ethereum L2 DeFi activity in 2023", layout="wide",initial_sidebar_state="collapsed")

# In[19]:


st.title('Ethereum L2 DeFi activity in 2023')


# In[20]:


st.markdown('This app shows the DeFi activity trends on **Ethereum L2** ecosystems since 2023. It is intended to provide an overview of the current activity on Layer 2 platforms.')


# In[5]:


st.markdown('To do that, we are gonna track the DeFi activity metrics registered on each L2 chain since the start of this year such as:') 
st.write('- Evolution of DeFi transactionsr')
st.write('- Evolution of DeFi users')
st.write('- Evolution of DeFi transactions per user')
st.write('- New DeFi users')
st.write('')
...
#st.markdown('In this first part, we can take a look at the main activity metrics on L2, where it can be seen how the number of transactions done across the protocol, as well as some other metrics such as fees and TPS.')
...
# Set y-axes titles
fig1.update_yaxes(title_text="Daily DeFi behavior", secondary_y=False)
...
# Set y-axes titles
fig2.update_yaxes(title_text="Daily DeFi behavior", secondary_y=False)
...
# Set y-axes titles
fig3.update_yaxes(title_text="Daily DeFi behavior", secondary_y=False)
...
df3.info()
#st.subheader('L2 general activity metrics regarding transactions')
#st.markdown('In this final part, we can take a look at the new users joining DeFi ecosystem on L2 as well as its main behavior activity.')
