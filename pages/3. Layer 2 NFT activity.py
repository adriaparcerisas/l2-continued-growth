...
st.set_page_config(page_title="Ethereum L2 NFT activity in 2023", layout="wide",initial_sidebar_state="collapsed")

# In[19]:


st.title('Ethereum L2 NFT activity in 2023')


# In[20]:


st.markdown('This app shows the NFT activity trends on **Ethereum L2** ecosystems before and after 2023. It is intended to provide an overview of the current activity on Layer 2 platforms.')


# In[5]:


st.markdown('To do that, we are gonna track the NFT activity metrics registered on each L2 chain since the start of this year such as:') 
st.write('- Evolution of NFT sales')
st.write('- Evolution of buyers')
st.write('- Evolution of NFT buys per user')
st.write('- New NFT purchasers')
st.write('')
...
# Set y-axes titles
fig1.update_yaxes(title_text="Daily NFT behavior", secondary_y=False)
...
# Set y-axes titles
fig2.update_yaxes(title_text="Daily NFT behavior", secondary_y=False)
...
# Set y-axes titles
fig3.update_yaxes(title_text="Daily NFT behavior", secondary_y=False)
...
df3.info()
#st.subheader('L2 general activity metrics regarding transactions')
#st.markdown('In this final part, we can take a look at the new users joining NFT ecosystem on L2 as well as its main behavior activity.')
